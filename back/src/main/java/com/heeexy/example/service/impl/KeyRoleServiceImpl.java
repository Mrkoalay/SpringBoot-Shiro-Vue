 ...
 */
@Service
public class KeyRoleServiceImpl implements KeyRoleService {
    @Autowired
    private KeyRoleDao keyRoleDao;
    ...
    public JSONObject listUser(JSONObject jsonObject) {
        ...
        int count = keyRoleDao.countUser(jsonObject);
        List<JSONObject> list = keyRoleDao.listUser(jsonObject);
    ...
    public JSONObject addUser(JSONObject jsonObject) {
        int exist = keyRoleDao.queryExistUsername(jsonObject);
        if (exist > 0) {
            return CommonUtil.errorJson(ErrorEnum.E_10009);
        }
        keyRoleDao.addUser(jsonObject);
    ...
    public JSONObject getAllRoles() {
        List<JSONObject> roles = keyRoleDao.getAllRoles();
    ...
    public JSONObject updateUser(JSONObject jsonObject) {
        keyRoleDao.updateUser(jsonObject);
        return CommonUtil.successJson();
    }
    ...
    public JSONObject listRole() {
        List<JSONObject> roles = keyRoleDao.listRole();
        return CommonUtil.successPage(roles);
    }
    ...
    public JSONObject listAllPermission() {
        List<JSONObject> permissions = keyRoleDao.listAllPermission();
    ...
    public JSONObject addRole(JSONObject jsonObject) {
        keyRoleDao.insertRole(jsonObject);
        keyRoleDao.insertRolePermission(jsonObject.getString("roleId"), (List<Integer>) jsonObject.get("permissions"));
        return CommonUtil.successJson();
    }
    ...
    public JSONObject updateRole(JSONObject jsonObject) {
        ...
        JSONObject roleInfo = keyRoleDao.getRoleAllInfo(jsonObject);
    ...
    private void dealRoleName(JSONObject paramJson, JSONObject roleInfo) {
        ...
        if (!roleName.equals(roleInfo.getString("roleName"))) {
            keyRoleDao.updateRoleName(paramJson);
        }
    }
    ...
    private void saveNewPermission(String roleId, Collection<Integer> newPerms, Collection<Integer> oldPerms) {
        ...
        if (waitInsert.size() > 0) {
            keyRoleDao.insertRolePermission(roleId, waitInsert);
        }
    }
    ...
    private void removeOldPermission(String roleId, Collection<Integer> newPerms, Collection<Integer> oldPerms) {
        ...
        if (waitRemove.size() > 0) {
            keyRoleDao.removeOldPermission(roleId, waitRemove);
        }
    }
    ...
    public JSONObject deleteRole(JSONObject jsonObject) {
        ...
        keyRoleDao.removeRole(jsonObject);
        keyRoleDao.removeRoleAllPermission(jsonObject);
        return CommonUtil.successJson();
    }
}
